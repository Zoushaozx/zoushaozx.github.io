...
```
精确匹配 {n} {n,m}    单参数 指定匹配数量 双参数 限定匹配范围
案例 A\d{3} == [A380] 只能匹配长度为3
		A\d{3，5} == [A380,A3800,A38000]  能够匹配长度 3，4，5
```
...
[123456789] 能够匹配 1～9 
[1-9] 能够匹配 1～9 
匹配长度为7～8的电话号码，不能够以0开头 
	[1-9]\d{6,7}
匹配大小不限的十六进制数(任意位置范围0-9-a-f，不区分大小写)，如1A2b3c
	[0-9a-fA-F]{6}
排除范围 [^1-9]{3} 表示匹配除0～9 以外的长度为三的内容 
	ABC Acd A_a
```
...
public class case1 {
    public static void main(String[] args) {
        //String re = "learn\\s(java|php|go)";  只能匹配小写
        String re = "learn\\s((J|j)ava|(P|p)hp|(G|g)o)";  //能够匹配大小写
...
```
引入 java.util.regex
使用Pattern 对象匹配 得到一个 Matcher对象 
Matcher.group(index) 返回子串
```
...
```
判断末尾0的个数
(\d+)(0*) 对于12300000 结果为  分组1：12300000  分组2: 
(\d+?)(0*) 对于12300000 结果为  分组1：123  分组2: 00000
当(\d+)加上？ (\d+?) 时就是非贪婪匹配
```
...
```
将搜索到的模版 按规则替换 
public class case8_reverse {
    public static void main(String[] args) {
        String s = "the quick brown fox jumps over the lazy dog";
        String replaceAll = s.replaceAll("\\s([a-z]{4})\\s", " <b>$1</b> ");
...
public class case9_module {
    public static void main(String[] args) {
        ...
        while (matcher.find()){
            ...
            matcher.appendReplacement(stringBuffer,Matcher.quoteReplacement(value.toString()));
        }
...
33 匹配特定字符前后的数字： /(\d+)(?=kg|公斤|Kg|KG|千克)/g
