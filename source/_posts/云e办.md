...
1⃣️准备包task新建类MailTask
	...
	注入
		...
		mailLogService
	设置定时任务
  	@Scheduled(cron = "0/10 * * * * ?") 邮件发送定时任务 10秒一次
  新建方法mailTask
  	判断重试条件
...
3⃣️测试
	...
	测试失败
		单次失败 
			修改EmployeeServiceImpl发送邮件信息参数 交换机名称
			丢失路由，发送失败，就会走定时任务，定时任务是正确的交换机名称
		多次失败
    	修改EmployeeServiceImpl发送邮件信息参数 交换机名称
			修改定时任务交换机名称。
```
...
3⃣️修改MailReceiver
  ...
  判断信息是否已经消费
  	hash.entries("mail_log").containsKey(msgId)
  	确认直接返回不做操作
4⃣️总结
	消息可靠性
		一方面，生产端保证消息被正确的送达，采用了消息落库，开启确认回调以及失败回调加上定时任务
		一方面，幂等性，在redis存入消息id，然后根据这个id去判断，如果id存在，说明这个数据已经被消费了。
```
...
```
1⃣️更改pojo类Salary属性时间格式
	在时间相关属性上加注解
    @JsonFormat(pattern = "yyyy-MM-dd",timezone = "Asia/shanghai")
2⃣️更改SalaryController
	注解路径“/salary/sob”
	增
		@PostMapping("/")
	删
